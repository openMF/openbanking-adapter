...
CREATE TABLE `payment_account_identification`
(
    `id`                BIGINT(20) NOT NULL AUTO_INCREMENT,
    ...
    CONSTRAINT `pk_payment_account_identification` PRIMARY KEY (id),
    CONSTRAINT `uk_payment_account_identification.link` UNIQUE (`payment_id`, `identification_id`),
    CONSTRAINT `fk_payment_account_identification.payment` FOREIGN KEY (`payment_id`) REFERENCES `payment` (`id`),
    CONSTRAINT `fk_payment_account_identification.ident` FOREIGN KEY (`identification_id`) REFERENCES `account_identification` (`id`)
);
